...
//copy constructor
    BinaryHeap(BinaryHeap &BH)
    {
        ...
        //copy array
        if (curSize<DEF_SIZE)
            array= new ElemType[DEF_SIZE];
        else array=new ElemType[curSize];
        for(int i=0; i<curSize; ++i)
            array[i]=BH.array[i];
    }
//assignment operator

    BinaryHeap operator = (BinaryHeap& BH)
    {
        if (array!=NULL)
            delete array; //delete old objects
...
template<typename ElemType, typename Comp>
void BinaryHeap<ElemType,Comp>::insert( ElemType& x)
{
...
void BinaryHeap<ElemType,Comp>::walkDown(int hole)
{
    ...
    {
        heap_comp+=2; //number of heap comparisons
        key_tmp+=2;
        child = 2*hole+1;
